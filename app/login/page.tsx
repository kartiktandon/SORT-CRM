      ...
      <section className="login-experience" aria-labelledby="login-title">
        <div className="login-lamp-side">
          <div className="lamp-scene">
            <div
              className={`lamp-toggle lamp-drag-control ${pullReady ? 'pull-ready' : ''}`}
            >
              <motion.div
                ...
                transition={{ duration }}
              />
              <motion.span className="lamp-fixture">
                <LampFloor size={340} strokeWidth={0.8} />
              </motion.span>
              <div className="lamp-cord-track">
                ...
                <motion.button
                  ...
                  aria-label={
                    lampOn
                      ? 'Pull cord to turn lamp off'
                      : 'Pull cord to turn lamp on'
                  }
                  aria-pressed={lampOn}
                  aria-controls="lamp-login-fields"
                  aria-describedby="lamp-pull-hint"
                  onDrag={(_event, info) => setPullReady(info.offset.y >= 40)}
                  onDragEnd={(event, info) => {
                    if (event.type !== 'pointercancel' && info.offset.y >= 40)
                      toggleLamp();
                    setPullReady(false);
                  }}
                  onClick={(event) => {
                    if (event.detail === 0) toggleLamp();
                  }}
                >
                  <Power size={16} />
                </motion.button>
              </div>
              <span className="lamp-switch-label" id="lamp-pull-hint">
                {pullReady
                  ? 'RELEASE TO SWITCH'
                  : lampOn
                    ? 'PULL DOWN TO TURN OFF'
          ...
          <div className="login-welcome">
            <span className="login-eyebrow">
              {lampOn
                ...
                : 'A LITTLE LIGHT. A FRESH START.'}
            </span>
            <h1 id="login-title">
              {lampOn ? 'You’re almost in' : 'Your workspace awaits'}
              <span>.</span>
            </h1>
            <p>
              {lampOn
                ? 'The light is on. Come in and make great things happen.'
                : 'Pull the cord down to light up your workspace and reveal sign-in.'}
            </p>
          </div>
        </div>
        <div className="lamp-form-stage">
            ...
            <AnimatePresence initial={false}>
              {lampOn && (
                <motion.div
                  ...
                  initial={{ opacity: 0, x: reduceMotion ? 0 : 110 }}
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: reduceMotion ? 0 : 110 }}
                  transition={{ duration }}
                >
                  <div className="login-card-heading">
                    ...
                    <div>
                      ...
                      <p>Pick up where the good work left off.</p>
                    </div>
                  </div>
                  <form
                  ...
                  <p className="login-availability">
                    Use your workspace account. Contact your administrator if
                    you need access.
                  </p>
                </motion.div>
              )}
          ...
          {!lampOn && (
            <div className="lamp-sleeping">
              <LockKeyhole size={21} />
              <p>Your sign-in is just a little light away.</p>
              <span>Drag the cord downward · or focus it and press Enter</span>
            </div>
          )}
        </div>
      </section>
